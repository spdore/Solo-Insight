...
import { Entry, ViewState, Achievement, Tag, SavedInsight, ContentItem, AiAccessState } from './types';
...
const ACHIEVEMENTS_LIST: Achievement[] = [
  { id: 'first_log', title: 'Awakening', description: 'Log your first session', iconName: 'flag', condition: (e) => e.length >= 1 },
  { id: 'week_streak', title: 'Rhythm', description: 'Log at least once in 7 days', iconName: 'flame', condition: (e) => e.length >= 7 }, 
  { id: 'explorer', title: 'Curiosity', description: 'Use 5 different tags', iconName: 'compass', condition: (e) => {
    const uniqueTags = new Set(e.flatMap(x => x.tags));
    return uniqueTags.size >= 5;
  }},
  { id: 'marathon', title: 'Deep Dive', description: 'Session longer than 30 mins', iconName: 'timer', condition: (e) => e.some(x => x.duration > 30) },
];
...
export default function App() {
  ...
  // AI Security State
  const [aiAccess, setAiAccess] = useState<AiAccessState>({ unlocked: false, attempts: 0 });
  const [isAiLockModalOpen, setIsAiLockModalOpen] = useState(false);
  ...
  const loadData = () => {
    ...
    setSavedInsights(StorageService.getSavedInsights());
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(FirebaseService.auth, async (currentUser) => {
        setUser(currentUser);
        StorageService.setUserId(currentUser ? currentUser.uid : null);
        
        if (currentUser) {
            setIsSyncing(true);
            
            // 1. Try to merge local data if this is a fresh cloud account
            const localData = StorageService.getFullBackupData();
            await FirebaseService.mergeLocalData(currentUser.uid, localData);

            // 2. Subscribe to Cloud Updates
            if (FirebaseService.db) {
                const unsubDb = onSnapshot(doc(FirebaseService.db, 'users', currentUser.uid), (doc) => {
                    const data = doc.data();
                    if (data) {
                        StorageService.syncFromCloud(data);
                        loadData(); // Refresh UI
                    }
                    setIsSyncing(false);
                });
                return () => unsubDb(); // Cleanup DB listener when auth changes
            }
        } else {
            // Guest mode
            loadData();
  ...
  const initiateAiGeneration = () => {
    if (aiAccess.unlocked) {
        generateInsight();
        return;
    }
    if (aiAccess.attempts >= 5) {
        alert("AI Insights are disabled due to repeated failed security attempts.");
        return;
    }
    setIsAiLockModalOpen(true);
  };
  ...
  const generateInsight = async () => {
    ...
    setAiInsight(null);
    setIsSaved(false);
    const result = await GeminiService.generateInsight(entries.slice(-20), "Last 20 entries");
    setAiInsight(result);
  ...
  const handleSaveInsight = () => {
    ...
    const newInsight: SavedInsight = {
        ...
        timestamp: Date.now(),
        text: aiInsight,
        period: "Last 20 entries"
    };
    const updated = StorageService.saveInsight(newInsight);
  ...
  const handleDeleteInsight = (id: string) => {
    const updated = StorageService.deleteInsight(id);
    setSavedInsights(updated);
  }
  ...
  const renderDashboard = () => (
    <div key="DASHBOARD" className="space-y-6 pb-24 animate-slide-up">
      <header className="flex justify-between items-center mb-6 pt-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
            Solo Insight
            {isSyncing && <Cloud size={14} className="text-violet-400 animate-pulse" />}
          </h1>
          <p className="text-slate-400 text-sm font-medium">Your personal rhythm</p>
        </div>
        <button 
            onClick={() => setIsSettingsOpen(true)}
            className={`p-3 rounded-full border transition-all ${user ? 'bg-violet-900/20 border-violet-500/30 text-violet-300' : 'bg-slate-900 border-slate-800 text-slate-400 hover:text-white'}`}
        >
            <Settings size={20} />
        </button>
      </header>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-slate-900/50 backdrop-blur-sm p-5 rounded-3xl border border-slate-800 shadow-xl">
            <div className="flex items-center gap-2 text-violet-400 mb-2">
                <Activity size={16} />
                <span className="text-[10px] uppercase font-bold tracking-widest text-slate-500">Frequency</span>
            </div>
            <div className="text-3xl font-light text-white">
                {stats.count30} <span className="text-sm text-slate-500 font-normal">/30 days</span>
            </div>
        </div>
        <div className="bg-slate-900/50 backdrop-blur-sm p-5 rounded-3xl border border-slate-800 shadow-xl">
             <div className="flex items-center gap-2 text-fuchsia-400 mb-2">
                <Clock size={16} />
                <span className="text-[10px] uppercase font-bold tracking-widest text-slate-500">Duration</span>
            </div>
            <div className="text-3xl font-light text-white">
                {stats.avgDuration} <span className="text-sm text-slate-500 font-normal">min avg</span>
            </div>
        </div>
      </div>
       ...
       <div className="relative overflow-hidden group rounded-3xl">
         ...
         <div className="relative p-6">
            <div className="flex justify-between items-start mb-4">
                <h3 className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Last Session</h3>
                {entries.length > 0 && <span className="text-xs font-medium px-2 py-1 rounded bg-slate-800 text-slate-300 border border-slate-700">{format(new Date(entries[entries.length-1].timestamp), 'MMM d')}</span>}
            </div>
            ...
            {entries.length > 0 ? (
                <div>
                    <div className="flex items-end gap-3 mb-2">
                        ...
                        <span className="text-sm text-slate-400 mb-1.5 font-medium">minutes</span>
                    </div>
                    <div className="flex gap-2 mt-4">
                        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-800/80 border border-slate-700 text-xs text-slate-300">
                             ...
                             {entries[entries.length-1].intensity}/5 Intensity
                        </div>
                ...
                <div className="text-slate-500 text-sm py-4">No entries recorded yet. Tap + to start.</div>
            )}
         </div>
       </div>
    </div>
  ...
  const renderCalendar = () => (
    <div key="CALENDAR" className="space-y-6 pb-24 h-full flex flex-col animate-slide-up">
      ...
      <div className="grid grid-cols-3 gap-3 animate-slide-up" style={{animationDelay: '0.05s'}}>
          <div className="bg-slate-900/50 p-3 rounded-2xl border border-slate-800 flex flex-col items-center justify-center">
              <span className="text-[10px] uppercase text-slate-500 font-bold tracking-wider mb-1">Sessions</span>
              <span className="text-xl font-light text-white">{monthStats.totalSessions}</span>
          </div>
          <div className="bg-slate-900/50 p-3 rounded-2xl border border-slate-800 flex flex-col items-center justify-center">
              <span className="text-[10px] uppercase text-slate-500 font-bold tracking-wider mb-1">Minutes</span>
              <span className="text-xl font-light text-white">{monthStats.totalDuration}</span>
          </div>
           <div className="bg-slate-900/50 p-3 rounded-2xl border border-slate-800 flex flex-col items-center justify-center">
              <span className="text-[10px] uppercase text-slate-500 font-bold tracking-wider mb-1">Avg Int</span>
                        ...
                        <SwipeableEntry key={entry.id} onDelete={() => handleDeleteEntry(entry.id)}>
                             <div className="bg-slate-900/50 border border-slate-800/50 p-4 rounded-2xl flex items-center gap-4 group relative">
                                 <div className="flex flex-col items-center justify-center w-12 h-12 rounded-xl bg-slate-800 text-slate-400 border border-slate-700 shrink-0">
                                     <span className="text-[10px] font-bold uppercase">{format(new Date(entry.timestamp), 'MMM')}</span>
                                 ...
                                 <div className="flex-1 min-w-0">
                                     ...
                                     {entry.tags.length > 0 && (
                                         <div className="flex gap-1 overflow-hidden">
                                             {entry.tags.slice(0,3).map(t => <span key={t} className="text-[10px] text-slate-500 truncate">#{t}</span>)}
                                         </div>
                                     )}
                                 </div>
                             </div>
      ...
      {selectedDay && (
          <div className="flex-1 bg-slate-900/90 backdrop-blur-xl rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.5)] border-t border-slate-800 p-6 animate-slide-up fixed bottom-0 left-0 right-0 z-40 max-w-md mx-auto">
              <div className="flex justify-between items-center mb-6">
                  <div>
                    <h3 className="font-bold text-xl text-white">{format(selectedDay.date, 'EEEE')}</h3>
                    <p className="text-slate-500 text-sm">{format(selectedDay.date, 'MMMM d, yyyy')}</p>
                  </div>
                                      ...
                                      {entry.tags.length > 0 && (
                                          <div className="flex flex-wrap gap-1.5 mb-2">
                                              {entry.tags.slice(0, 4).map(t => (
                                                  <span key={t} className="text-[10px] font-medium bg-slate-800 text-slate-300 px-2 py-1 rounded-md border border-slate-700">{t}</span>
                                              ))}
                                          </div>
                                      )}
      ...
      {view === 'LIBRARY' && <Library items={libraryItems} onUpdate={setLibraryItems} />}
  ...
  const renderStats = () => (
    <div key="STATS" className="space-y-6 pb-24 animate-slide-up">
        {/* Same stats code as before, simplified for this snippet */}
        <h1 className="text-2xl font-bold tracking-tight text-white pt-4">Deep Insights</h1>
        
        {/* ... (Existing AI Card & Stats Grid) ... */}
        {/* Placeholder to keep the file valid if you copy paste fully, but reusing the existing components is fine */}
         <div className="bg-gradient-to-br from-indigo-900 to-violet-950 text-white p-6 rounded-3xl shadow-xl border border-indigo-800 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-fuchsia-500/10 blur-3xl rounded-full pointer-events-none -mr-10 -mt-10"></div>
          
          <div className="flex justify-between items-start mb-4 relative z-10">
            <div className="flex items-center gap-2 text-indigo-200">
                <Sparkles size={18} className="text-indigo-300" />
                <span className="text-[10px] font-bold uppercase tracking-widest">AI Pattern Analysis</span>
            </div>
            
            {aiInsight && !loadingInsight && (
              <div className="flex gap-2">
                 <button onClick={initiateAiGeneration} className="p-2 bg-indigo-800/50 hover:bg-indigo-700/50 rounded-full transition-colors text-indigo-300" title="Regenerate">
                    <RefreshCw size={16} />
                 </button>
                 <button onClick={handleSaveInsight} className="p-2 bg-indigo-800/50 hover:bg-indigo-700/50 rounded-full transition-colors text-indigo-300" title="Save">
                    {isSaved ? <Check size={16} className="text-emerald-400"/> : <Bookmark size={16} />}
                 </button>
              </div>
            )}
          </div>
          ...
          <div className="relative z-10 min-h-[80px]">
            {loadingInsight ? (
                <div className="animate-pulse space-y-3">
                    ...
                    <div className="h-2 bg-indigo-400/30 rounded w-2/3"></div>
                </div>
            ) : aiInsight ? (
                <p className="text-sm leading-relaxed text-indigo-50 font-light border-l-2 border-indigo-400/50 pl-3 animate-in fade-in">
                    {aiInsight}
                </p>
            ) : (
                <div className="text-center py-2">
                    <p className="text-xs text-indigo-300 mb-4 leading-relaxed">Unlock personalized patterns regarding your timing, duration, and preferences.</p>
                    {aiAccess.attempts >= 5 ? (
                         <div className="flex items-center justify-center gap-2 text-red-400 bg-red-950/30 py-2 px-4 rounded-lg border border-red-900/50">
                             <Lock size={14} />
                             <span className="text-xs font-bold uppercase tracking-wider">Feature Locked</span>
                         </div>
                    ) : (
                        <button 
                            onClick={initiateAiGeneration}
                            className="px-5 py-2.5 bg-white text-indigo-950 text-xs font-bold uppercase tracking-wide rounded-full hover:bg-indigo-50 transition-colors shadow-lg"
                        >
                            Reveal Insights
                        </button>
                    )}
       ...
       <div className="grid grid-cols-2 gap-4">
          <div className="bg-slate-900 p-5 rounded-3xl border border-slate-800">
              <div className="text-[10px] text-slate-500 uppercase font-bold mb-2 tracking-widest">Climax Rate</div>
          ...
          <div className="bg-slate-900 p-5 rounded-3xl border border-slate-800">
              <div className="text-[10px] text-slate-500 uppercase font-bold mb-2 tracking-widest">Avg Intensity</div>
  ...
  const renderAchievements = () => (
      <div key="ACHIEVEMENTS" className="space-y-6 pb-24 animate-slide-up">
        <h1 className="text-2xl font-bold tracking-tight text-white mb-6 pt-4">Milestones</h1>
                        ...
                        <div className="relative z-10">
                            ...
                            {isUnlocked && (
                                <p className="text-[10px] text-violet-400 mt-2 uppercase font-bold tracking-wider flex items-center gap-1">
                                    Unlocked {format(new Date(unlockedAchievements[ach.id]), 'MMM d')}
                                </p>
                            )}
                        </div>
                    </div>
  ...
  return (
    <div className="min-h-screen w-full flex justify-center bg-black">
        <div className="w-full max-w-md bg-slate-950 relative overflow-hidden flex flex-col shadow-2xl">
        ...
        {showSuccess && (
            <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300 pointer-events-none">
                ...
                <h2 className="mt-8 text-2xl font-bold text-white tracking-tight animate-in slide-in-from-bottom-4 duration-500">Recorded</h2>
                <p className="text-slate-400 mt-2 font-medium">Session saved successfully</p>
            </div>
        )}
        ...
        <nav className="absolute bottom-0 left-0 right-0 bg-slate-950/80 backdrop-blur-md border-t border-slate-800 px-6 py-4 z-20 grid grid-cols-5 items-center text-xs font-medium">
            ...
            <span className={view === 'DASHBOARD' ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden'}>Today</span>
            </button>
            <button 
            ...
            <span className={view === 'CALENDAR' ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden'}>History</span>
            </button>
            <button 
            ...
            <span className={view === 'LIBRARY' ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden'}>Lib</span>
            </button>
            <button 
            ...
            <span className={view === 'STATS' ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden'}>Insights</span>
            </button>
            <button 
            ...
            <span className={view === 'ACHIEVEMENTS' ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden'}>Awards</span>
            </button>
        </nav>
        ...
        />
        <AiLockModal 
            isOpen={isAiLockModalOpen} 
            onClose={() => setIsAiLockModalOpen(false)} 
            onSuccess={() => generateInsight()}
